            ...
            if local_rank == 0:
                ...
                if global_step % 100 == 0:
                    for name, value in loss_stats_rgb.items():
                        writer.add_scalar(f"RGBLoss/{name}", value, global_step)
                    for name, value in loss_stats_thermal.items():
                        writer.add_scalar(f"ThermalLoss/{name}", value, global_step)
                    for name, value in loss_stats_output.items():
                        writer.add_scalar(f"OutputLoss/{name}", value, global_step)
